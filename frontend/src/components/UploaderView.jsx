...
export default function UploaderView({ country, weeks, selectedWeek, setSelectedWeek, onBack }) {
  ...
  const { isLocked, extendedUntil, extensionStatus } = (() => {
    ...
    let extUntil = null;
    let extStatus = null; // 'pending' | 'approved' | null

    if (locked && delaysData) {
      if (delaysData.global && delaysData.global.extendedUntil) {
        extUntil = new Date(delaysData.global.extendedUntil);
        extStatus = 'approved';
      }
      if (delaysData.requests && delaysData.requests[country.id]) {
        const req = delaysData.requests[country.id];
        if (req.status === 'approved') {
          const reqUntil = new Date(req.extendedUntil);
          if (!extUntil || reqUntil > extUntil) {
            extUntil = reqUntil;
          }
          extStatus = 'approved';
        } else if (req.status === 'pending' && !extUntil) {
          extStatus = 'pending';
        }
      }
      if (extUntil && new Date() <= extUntil) {
        locked = false;
      }
    }
