...
export const OVERLAY_TEMPLATES = [
  {
    ...
    build({ name, title }, fontPath) {
      ...
      const f = fontPath.replace(/:/g, '\\\\:');
      return [
        // White background bar
        `drawbox=x=0:y=ih-120:w=iw*0.55:h=70:color=0x000000AA:t=fill`,
        // Name (large, bold)
        `drawtext=fontfile='${f}':text='${safe(name)}':fontcolor=white:fontsize=36:x=20:y=h-105:shadowcolor=black:shadowx=2:shadowy=2`,
        // Title (smaller, accent yellow)
        `drawtext=fontfile='${f}':text='${safe(title)}':fontcolor=0xFFD700:fontsize=22:x=20:y=h-65:shadowcolor=black:shadowx=1:shadowy=1`,
      ];
    },
  },
  {
    ...
    build({ title, date }, fontPath) {
      ...
      return [
        ...
        // Main title
        `drawtext=fontfile='${f}':text='${safe(title)}':fontcolor=white:fontsize=60:x=(w-text_w)/2:y=(h-text_h)/2-20:shadowcolor=black:shadowx=3:shadowy=3`,
        // Subtitle/date
        `drawtext=fontfile='${f}':text='${safe(date)}':fontcolor=0xFFD700:fontsize=28:x=(w-text_w)/2:y=(h-text_h)/2+50:shadowcolor=black:shadowx=2:shadowy=2`,
      ];
    },
  },
  {
    ...
    build({ pays }, fontPath) {
      ...
      return [
        ...
        `drawtext=fontfile='${f}':text='${safe(pays)}':fontcolor=white:fontsize=30:x=iw-220:y=30:shadowcolor=black:shadowx=2:shadowy=2`,
      ];
    },
  },
  {
    ...
    build({ sujet }, fontPath) {
      ...
      return [
        ...
        `drawtext=fontfile='${f}':text='${safe(sujet)}':fontcolor=white:fontsize=32:x=20:y=h-60:shadowcolor=black:shadowx=2:shadowy=2`,
      ];
    },
  },
];
